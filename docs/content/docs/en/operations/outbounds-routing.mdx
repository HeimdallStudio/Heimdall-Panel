...
description: Shape egress in Heimdall — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
Heimdall can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes Heimdall uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. Heimdall can register a
...
From the panel's WARP controls, register an account. Heimdall fills the outbound's
...
Heimdall can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
